...
from straph.dag import stable_dag as cdag
    ...
    with open(storage_file + 'comp.scf', 'rb') as input:
        ...
        for l in unpacker:
            if l[0] == (id_wcc, id_scc):
                return stable_connected_component(id=id_scc, times=l[1],
                                                  links=l[2])
    ...
    with open(storage_path + "scc_" + str(scc_id) + '.sgf', 'rb') as input:
        ...
        for i in input:
            ...
            if len(l) <= 2:
                ...
                links.append(l)
    return stable_connected_component(id=scc_id,
                                      times=(t0, t1),
                                      links=links)
...
def compute_stable_connected_components(S, format="object_with_links", stable_dag=False, isolated_nodes=True,
    ...
    '''
    Compute Stable Connectec Components (SCC) of a Stream Graph.
    ...
    if isolated_nodes:
        for c in S.get_isolated_nodes():
            if format == "cluster":
                ...
                c = stable_connected_component(id=cnt_scc_id,
                                               times=[c[0], c[1]],
                                               nodes={c[2]})
                final_components.append(c)
                if stable_dag:
...
def process_arrival(l, node_2_status, tmp_components, final_components, stable_dag,
...
def create_scc(l, node_2_status, tmp_components, format="cluster"):
    ...
    if format == "object_with_links":
        ...
        lks = None
    tmp_components.append(stable_connected_component(times=[t0, t0],
                                                     nodes={u, v},
                                                     set_links={(u, v)},
                                                     links=lks))
    # print("creation :",{u,v})
...
class stable_connected_component:
    def __init__(self,
                 id=None,
                 times=None,
                 nodes=None,
    ...
    def __copy__(self):
        ...
        return stable_connected_component(times=t,
                                          nodes=n,
                                          links=l)
        ...
        if sum([1 for el in component_2_set_links if el is not None]) > 1:
            for set_links in component_2_set_links:
                if set_links:
                    ...
                    new_c = stable_connected_component(set_links=set_links,
                                                       nodes=current_nodes,
                                                       links=current_links)
                    R.append(new_c)
        return R
