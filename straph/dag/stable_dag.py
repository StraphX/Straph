...
class stable_dag:
    ...
    def core_number(self, n_jobs=-1):
        ...
        r = Parallel(n_jobs=n_jobs, mmap_mode='r+')(
            delayed(para_cores)(comp) for comp in self.c_nodes if comp.size() > 1)
        for l in r:
            for k, v in l.items():
                L[k] += v
        return L
    ...
    def all_cliques(self, n_jobs=-1):
        ...
        def para_cliques(comp):
            return comp.all_cliques()

        r = Parallel(n_jobs=n_jobs, mmap_mode='r+')(
            delayed(para_cliques)(comp) for comp in self.c_nodes if comp.size() > 1)
        for l in r:
            for k, v in l.items():
                L[k] += v
        return L
    ...
    ####################################################
    # TODO: Need to update functions in 1. with functions in 2.
